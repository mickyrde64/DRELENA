...
export const Services: React.FC = () => {
  ...
  const getIcon = (name: string) => {
    switch(name) {
      ...
      case 'shield': return <ShieldPlus className="w-8 h-8 text-teal-600" />;
      case 'users': return <Users className="w-8 h-8 text-teal-600" />;
      case 'activity': return <Activity className="w-8 h-8 text-teal-600" />;
      case 'smile': return <Stethoscope className="w-8 h-8 text-teal-600" />;
      case 'heart': return <Heart className="w-8 h-8 text-teal-600" />;
      default: return <Activity className="w-8 h-8 text-teal-600" />;
    }
  };
          ...
          {services.map((service) => (
            ...
            >
              <div className="w-14 h-14 bg-teal-50 rounded-xl flex items-center justify-center mb-6 group-hover:bg-teal-600 group-hover:text-white transition-colors duration-300">
                {React.cloneElement(getIcon(service.icon) as React.ReactElement, {
                  className: `w-7 h-7 transition-colors duration-300 group-hover:text-white text-teal-600`
                })}
              </div>
